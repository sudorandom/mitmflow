...
import { ContentFormat, formatBytes, formatDuration, getFlowId, getRequest, getTimestamp } from './utils';
        ...
        if (flow?.flow?.case === 'httpFlow') {
          ...
          return [{
            ...
            request: {
              ...
              queryString: httpFlow.request?.url ? new URL(httpFlow.request.url).searchParams : [],
              cookies: [],
...
const App: React.FC = () => {
  ...
  const columns = useMemo<MRT_ColumnDef<Flow>[]>(() => [
    {
      ...
      header: '',
      size: 40,
      Cell: ({ row }) => <FlowIcon flow={row.original} />,
    },
    {
      id: 'status',
      header: 'Status',
      size: 80,
      Cell: ({ row }) => <StatusPill flow={row.original} />,
    },
    {
      id: 'request',
      header: 'Request',
      accessorFn: (flow) => getRequest(flow),
    },
    {
      id: 'transfer',
      header: 'Transfer',
      size: 120,
      Cell: ({ row }) => {
        if (row.original.flow.case === 'httpFlow') {
          const httpFlow = row.original.flow.value;
          const size = (httpFlow.response?.content?.length || 0);
    ...
    },
    {
      ...
      accessorFn: (flow) => {
        if (flow.flow.case === 'httpFlow') {
          return formatDuration(flow.flow.value.durationMs);
        }
        return '';
      }
    }
  ...
  const selectedFlowId = selectedFlow ? getFlowId(selectedFlow) : null;
              ...
              {isMenuOpen && (
                <div className="absolute right-0 mt-2 w-48 bg-zinc-800 border border-zinc-700 rounded-md shadow-lg z-20">
                  ...
                  <div className="relative inline-block w-full" ref={bulkDownloadRef}>
                    ...
                    >
                      <Download size={20} /> Download ({numSelected})
                    </button>
                    {isBulkDownloadOpen && (
