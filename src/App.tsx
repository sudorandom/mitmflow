...
const App: React.FC = () => {
  ...
  const handleFlowMouseDown = useCallback((flow: Flow, event?: React.MouseEvent) => {
    ...
    if (event?.shiftKey && lastSelectedFlowId.current) {
      ...
      // Default case: clear previous selections and select the current one.
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Enter' || e.key === ' ') {
        if (focusedFlowId) {
          e.preventDefault();
          const flowToSelect = filteredFlows.find(f => getFlowId(f) === focusedFlowId);
          if (flowToSelect) {
            handleFlowMouseDown(flowToSelect, e.shiftKey ? { shiftKey: true } as React.MouseEvent : undefined);
          }
        }
        return;
      }
